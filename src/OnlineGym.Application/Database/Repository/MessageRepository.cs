...
public class MessageRepository : IMessageRepository
{
    public long Insert(Message message)
    {
        ...
        if (result is null || result == DBNull.Value)
        {
            throw new InvalidOperationException(
                "Poruka nije sačuvana.");
        }

        return Convert.ToInt64(result);
    }

    public List<Message> GetByRecipientAccountId(
    ...
    private static Message MapFromReader(IDataReader reader)
    {
        return new Message(
            reader.GetInt64(
                reader.GetOrdinal("message_id")),
            reader.GetInt64(
                reader.GetOrdinal("sender_account_id")),
            reader.GetInt64(
                reader.GetOrdinal("recipient_account_id")),
            reader.GetString(
                reader.GetOrdinal("content")),
            reader.GetBoolean(
                reader.GetOrdinal("is_read")),
            reader.GetDateTime(
                reader.GetOrdinal("sent_at")));
    }
}
